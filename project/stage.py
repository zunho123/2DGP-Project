...
class Stage:
    def __init__(self, bg_path, window_w, window_h, zoom_mul=1.0, ground_ratio=0.0, ground_px=0):
    ...
    def set_view_by_center(self, x, lead_px=0):
        center = x + lead_px
        left = center - self.vw / 2
        left = max(0, min(left, max(0, self.bg_w - self.vw)))
        self.left = left
        self.bottom = 0
